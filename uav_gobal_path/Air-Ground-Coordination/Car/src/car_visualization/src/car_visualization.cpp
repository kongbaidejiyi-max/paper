...
static inline string resolvedFrame(const std_msgs::Header& header)
{
  ...
  ROS_WARN_THROTTLE(1.0, "car_visualization: odom.header.frame_id is empty; defaulting marker frame to 'odom' (set ~output_frame to override).");
  return "odom";
}
...
static void odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  if ((msg->header.frame_id.empty() || msg->header.frame_id == string("null")) && output_frame.empty())
  {
    ROS_WARN_THROTTLE(1.0,
                      "car_visualization: ignoring odom because header.frame_id is empty/'null' and ~output_frame is empty; set ~output_frame (e.g. 'world').");
    return;
  }
  ...
  ROS_DEBUG_THROTTLE(2.0, "car_visualization: publishing markers in frame '%s' from odom frame '%s'", frame.c_str(), msg->header.frame_id.c_str());
  ...
  if (resolve_mesh_resource)
  {
    ...
    if (resolved == mesh_resource && mesh_resource.rfind("package://", 0) == 0)
    {
      ROS_WARN_THROTTLE(2.0,
                        "car_visualization: could not resolve mesh_resource '%s' (package not found); RViz may fail to load it",
                        mesh_resource.c_str());
    }
...
{
  ros::init(argc, argv, "car_visualization");
  ros::NodeHandle nh;
  ros::NodeHandle pnh("~");

  pnh.param("mesh_resource", mesh_resource, std::string("package://car_visualization/meshes/car.dae"));
  ...
  ROS_INFO("car_visualization: subscribing to odom: %s", odom_topic.c_str());
  ROS_INFO("car_visualization: output_frame: '%s'", output_frame.empty() ? "(use odom header.frame_id)" : output_frame.c_str());
