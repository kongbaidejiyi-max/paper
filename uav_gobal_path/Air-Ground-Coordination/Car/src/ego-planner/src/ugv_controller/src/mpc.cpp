...
void MPC::init(ros::NodeHandle &nh)
{
    ...
    pos_cmd_pub_ = nh.advertise<geometry_msgs::Twist>("/car/cmd_vel", 200);
    ...
    odom_sub_ = nh.subscribe("/car/Odometry", 1, &MPC::rcvOdomCallBack, this, ros::TransportHints().tcpNoDelay());
...
void MPC::bsplineCallback(ego_planner::BsplineConstPtr msg)
{
    // parse pos traj
    ...
    true_start_time_ = ros::Time::now();
    start_time_ = true_start_time_ = msg->start_time;

    // start_time_      = true_start_time_;   // 或者干脆就只用一个 start_time_
...
void MPC::cmdCallback(const ros::TimerEvent &e)
{
    ...
    // ROS_INFO("in MPC, the cmd is: a=%f, steer=%f", cmd.acceleration, cmd.steering_angle);
}
