...
namespace ego_planner
{
  ...
  public:
    ...
    void setGRPGuide(const std::vector<Eigen::Vector3d> &pts, const std::vector<float> &half_width,
  ...
  private:
    ...
    bool grp_has_guide_{false};
    std::vector<Eigen::Vector3d> grp_pts_;
    std::vector<float> grp_half_width_;
    std::vector<float> grp_confidence_;
    double grp_lambda_{0.0};
    double grp_outside_w_{1.0};
    double grp_centerline_w_{0.1};
    ...
    void calcGRPCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient);
