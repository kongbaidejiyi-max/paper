...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("grp/cost_outside_weight", grp_outside_w_, 1.0);
    nh.param("grp/cost_centerline_weight", grp_centerline_w_, 0.1);
  }

  void BsplineOptimizer::setGRPGuide(const std::vector<Eigen::Vector3d> &pts, const std::vector<float> &half_width,
                                     const std::vector<float> &confidence, double lambda)
  {
    grp_pts_ = pts;
    grp_half_width_ = half_width;
    grp_confidence_ = confidence;
    grp_lambda_ = std::min(1.0, std::max(0.0, lambda));
    grp_has_guide_ = (grp_pts_.size() >= 2 && grp_lambda_ > 1e-6);
  }

  void BsplineOptimizer::clearGRPGuide()
  {
    grp_has_guide_ = false;
    grp_lambda_ = 0.0;
    grp_pts_.clear();
    grp_half_width_.clear();
    grp_confidence_.clear();
  }
  ...
  void BsplineOptimizer::calcGRPCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    cost = 0.0;
    if (!grp_has_guide_ || grp_pts_.size() < 2)
      return;

    const int end_idx = q.cols() - order_;

    auto widthAt = [&](int seg_idx, double seg_t) -> double
    {
      if (grp_half_width_.empty())
        return std::max(0.0, dist0_);
      const int i0 = std::min(std::max(seg_idx, 0), (int)grp_half_width_.size() - 1);
      const int i1 = std::min(i0 + 1, (int)grp_half_width_.size() - 1);
      const double w0 = std::max(0.0, (double)grp_half_width_[i0]);
      const double w1 = std::max(0.0, (double)grp_half_width_[i1]);
      return w0 + seg_t * (w1 - w0);
    };

    auto confAt = [&](int seg_idx, double seg_t) -> double
    {
      if (grp_confidence_.empty())
        return 1.0;
      const int i0 = std::min(std::max(seg_idx, 0), (int)grp_confidence_.size() - 1);
      const int i1 = std::min(i0 + 1, (int)grp_confidence_.size() - 1);
      const double c0 = std::min(1.0, std::max(0.0, (double)grp_confidence_[i0]));
      const double c1 = std::min(1.0, std::max(0.0, (double)grp_confidence_[i1]));
      return c0 + seg_t * (c1 - c0);
    };

    for (int i = order_; i < end_idx; ++i)
    {
      const Eigen::Vector3d p = q.col(i);

      double best_d2 = std::numeric_limits<double>::infinity();
      int best_seg = 0;
      double best_t = 0.0;
      Eigen::Vector3d best_proj = grp_pts_.front();

      for (int s = 0; s < (int)grp_pts_.size() - 1; ++s)
      {
        const Eigen::Vector3d a = grp_pts_[s];
        const Eigen::Vector3d b = grp_pts_[s + 1];
        const Eigen::Vector3d d = b - a;
        const double denom = d.squaredNorm();
        double t = 0.0;
        if (denom > 1e-12)
          t = (p - a).dot(d) / denom;
      ...
      const Eigen::Vector3d diff = p - best_proj;
      const double conf = confAt(best_seg, best_t);
      const double width = widthAt(best_seg, best_t);

      if (grp_centerline_w_ > 1e-9 && conf > 1e-6)
      {
        cost += grp_centerline_w_ * conf * best_d2;
        gradient.col(i) += 2.0 * grp_centerline_w_ * conf * diff;
      }

      const double dist = std::sqrt(std::max(0.0, best_d2));
      if (grp_outside_w_ > 1e-9 && conf > 1e-6 && dist > std::max(0.0, width) + 1e-6)
      {
        const double r = dist - std::max(0.0, width);
        cost += grp_outside_w_ * conf * r * r;
        gradient.col(i) += 2.0 * grp_outside_w_ * conf * (r / std::max(1e-6, dist)) * diff;
      }
    }
  }
  ...
  void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    double f_smoothness, f_distance, f_feasibility, f_grp;

    Eigen::MatrixXd g_smoothness = Eigen::MatrixXd::Zero(3, cps_.size);
    Eigen::MatrixXd g_distance = Eigen::MatrixXd::Zero(3, cps_.size);
    Eigen::MatrixXd g_feasibility = Eigen::MatrixXd::Zero(3, cps_.size);
    Eigen::MatrixXd g_grp = Eigen::MatrixXd::Zero(3, cps_.size);

    calcSmoothnessCost(cps_.points, f_smoothness, g_smoothness);
    calcDistanceCostRebound(cps_.points, f_distance, g_distance, iter_num_, f_smoothness);
    calcFeasibilityCost(cps_.points, f_feasibility, g_feasibility);
    calcGRPCost(cps_.points, f_grp, g_grp);

    const double mix = grp_has_guide_ ? grp_lambda_ : 0.0;
    f_combine = lambda1_ * f_smoothness + (1.0 - mix) * new_lambda2_ * f_distance + lambda3_ * f_feasibility + mix * f_grp;
    //printf("origin %f %f %f %f\n", f_smoothness, f_distance, f_feasibility, f_combine);

    Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + (1.0 - mix) * new_lambda2_ * g_distance + lambda3_ * g_feasibility + mix * g_grp;
