...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    nh.param("grp/enable", grp_enable_, false);
    nh.param("grp/forward_len", grp_forward_len_, 15.0);
    grp_manager_.setParam(nh);
  ...
  void EGOPlannerManager::updateGRPPath(const fitplane_planner::GlobalPath &msg)
  {
    grp_manager_.update(msg);
  }

  bool EGOPlannerManager::getLocalTargetFromGRP(const Eigen::Vector3d &current_pos, Eigen::Vector3d &local_target_pt, Eigen::Vector3d &local_target_vel) const
  {
    if (!grp_enable_ || !grp_manager_.valid())
      return false;
    return grp_manager_.getLocalTarget(current_pos, pp_.planning_horizen_, pp_.max_vel_, pp_.max_acc_, local_target_pt, local_target_vel);
  }
...
cout << "reboundReplan.............." << endl;
    ...
    if ((start_pt - local_target_pt).norm() < 0.2) //FIXME
    {
    ...
    if (grp_enable_ && grp_manager_.valid())
    {
      std::vector<Eigen::Vector3d> grp_pts;
      std::vector<float> grp_w;
      std::vector<float> grp_c;
      double conf_mean = 0.0, unknown_rate = 0.0, conflict_rate = 0.0;
      const double lambda = grp_manager_.computeAdaptiveLambda(grid_map_, start_pt, pp_.planning_horizen_, &conf_mean, &unknown_rate, &conflict_rate);
      if (lambda > 1e-6 && grp_manager_.exportSubPath(start_pt, grp_forward_len_, grp_pts, grp_w, grp_c))
        bspline_optimizer_rebound_->setGRPGuide(grp_pts, grp_w, grp_c, lambda);
      else
        bspline_optimizer_rebound_->clearGRPGuide();
    }
    else
    {
      bspline_optimizer_rebound_->clearGRPGuide();
    ...
    do
    {
      ...
      //     PolynomialTraj gl_traj = PolynomialTraj::one_segment_traj_gen(start_pt,
      //                                                                   start_vel,
      //                                                                   start_acc,
      //                                                                   local_target_pt, local_target_vel, Eigen::Vector3d::Zero(), poly_time);
    ...
    {
      visualization_->displayOptimalList( ctrl_pts, vis_id ); //FIXME
      continous_failures_count_++;
      return false;
    }
    ...
    UniformBspline pos = UniformBspline(ctrl_pts, 3, ts); //FIXME ？ 
