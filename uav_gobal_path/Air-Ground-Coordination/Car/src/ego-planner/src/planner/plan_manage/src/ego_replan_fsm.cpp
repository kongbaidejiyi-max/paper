...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    nh.param("grp/topic", grp_topic_, std::string("/grp"));
    ...
    /* callback */
    exec_timer_ = nh.createTimer(ros::Duration(0.5), &EGOReplanFSM::execFSMCallback, this);
    safety_timer_ = nh.createTimer(ros::Duration(2), &EGOReplanFSM::checkCollisionCallback, this);
    go_timer_ = nh.createTimer(ros::Duration(0.01), &EGOReplanFSM::goFlagCallback, this);

    goFlagPub_ = nh.advertise<std_msgs::Int16>("go_flag",10);

    odom_sub_ = nh.subscribe("/odom_world", 1, &EGOReplanFSM::odometryCallback, this);
    if (grp_enable_ || target_type_ == TARGET_TYPE::REFENCE_PATH)
      grp_sub_ = nh.subscribe(grp_topic_, 1, &EGOReplanFSM::grpCallback, this);
  ...
  void EGOReplanFSM::grpCallback(const fitplane_planner::GlobalPathConstPtr &msg)
  {
    if (msg == nullptr || msg->poses.size() < 2)
      return;

    planner_manager_->updateGRPPath(*msg);
    ...
    const auto &p_end = msg->poses.back().position;
    end_pt_ << p_end.x, p_end.y, 0.0;
    end_vel_.setZero();

    std::vector<Eigen::Vector3d> global_path;
    global_path.reserve(msg->poses.size());
    for (const auto &pose : msg->poses)
    {
      global_path.emplace_back(pose.position.x, pose.position.y, 0.0);
    }
    visualization_->displayGlobalPathList(global_path, 0.1, 0);

    if (exec_state_ == WAIT_TARGET)
      changeFSMExecState(GEN_NEW_TRAJ, "GRP");
    else if (exec_state_ == EXEC_TRAJ)
      changeFSMExecState(REPLAN_TRAJ, "GRP");
  }
void EGOReplanFSM::goFlagCallback(const ros::TimerEvent &e)
{
  ...
  void EGOReplanFSM::waypointCallback(const nav_msgs::PathConstPtr &msg)
  {
    ...
    bool success = false;
    end_pt_ << msg->poses[0].pose.position.x, msg->poses[0].pose.position.y, 0; //twilight: goal height
...
// FIXME
  void EGOReplanFSM::getLocalTarget()
  {
    if (target_type_ == TARGET_TYPE::REFENCE_PATH)
    {
      if (!planner_manager_->getLocalTargetFromGRP(start_pt_, local_target_pt_, local_target_vel_))
      {
