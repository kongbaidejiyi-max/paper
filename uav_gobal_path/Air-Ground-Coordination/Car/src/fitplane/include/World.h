...
public:
     ...
     * @return bool true(grid map built successfully), false(grid map build failed)
     */
    bool initGridMap(const pcl::PointCloud<pcl::PointXYZ> &cloud);
     ...
     * @return bool true(grid map built successfully), false(grid map build failed)
     */
    bool initGridMap(const Eigen::Vector3d &lowerbound,const Eigen::Vector3d &upperbound);
    ...
    std::string robot_pose_topic = "/odom";
    ...
    geometry_msgs::Quaternion ego_orientation_;
    ...
    void GetRobotPosition()
    {
        ...
        try
        {
            ...
            ego_orientation_ = transform_pose.transform.rotation;
