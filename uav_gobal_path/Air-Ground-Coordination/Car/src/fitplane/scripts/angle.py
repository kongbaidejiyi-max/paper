...
class TraversabilityAngleVisualizer:
    def __init__(self):
        ...
        self.fig.suptitle('Traversability and Surface Angle Analysis', fontsize=16)
        ...
        try:
            ...
            for i, (ax, data, cmap, vlim, cb_label) in enumerate(zip(self.axes, data_list, cmaps, vlims, colorbar_labels)):
                ...
                if len(valid_data) > 0:
                    ...
                    if i == 0:  # 可通行性
                        title_text = (f'Traversability (Higher = Less Traversable)\n'
                                    f'Cells: {count}, Mean: {mean_val:.3f}, '
                                    f'Range: [{min_val:.3f}, {max_val:.3f}]')
                    else:  # 角度
                        title_text = (f'Surface Angle (Higher = Steeper)\n'
